...
/**
 * POST /api/admin/achievements/bulk/activate
 * Bulk activate multiple achievements
 */
export async function POST(request) {
  try {
    ...
    // Bulk activate
    const result = await achievementAdminService.bulkActivate(
      achievementIds,
      session.user.id,
      ipAddress,
      userAgent
    ...
    return new Response(
      JSON.stringify({
        message: `Successfully activated ${result.modifiedCount} of ${result.matchedCount} achievements`,
        ...result
      }),
      { status: 200, headers: responseHeaders }
    );
  } catch (error) {
    console.error('POST /api/admin/achievements/bulk/activate error:', error);
